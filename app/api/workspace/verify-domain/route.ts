...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      // Check if any of the CNAMEs match the target
      // Note: resolveCname returns an array of CNAME records
      const isValid = cnames.some(cname => cname === target);

      if (isValid) {
        return NextResponse.json({ valid: true, message: "Domain is correctly configured" });
      } else {
        return NextResponse.json({ 
          valid: false, 
      ...
      // DNS resolution failed (e.g., no CNAME record)
